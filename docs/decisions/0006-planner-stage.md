...
The mitigation is that **planner lag is the primary health metric**: `max(now() - received_at) WHERE planned_at IS NULL`, exposed on `/api/readyz`, which reports `degraded` past 60 seconds. Anyone operating hookfan should alert on it. A gateway that accepts webhooks and quietly forwards none is the worst failure this system can have, and lag is the only signal that distinguishes it from a quiet day.
